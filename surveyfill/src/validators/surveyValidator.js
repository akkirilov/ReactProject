function validateNewSurvey(survey) { 
    ...
    for (let i = 0; i < sectionsLength; i++) {
        if (survey.sections[i].sectionTitle.length < 5) {
            return "Section title must has at least 5 symbols!";
        }
        ...
        for (let j = 0; j < questionsLength; j++) {
            if (questions[j].questionTitle.length < 5) {
                return "Question title must has at least 5 symbols!";
            }
            let possibilities = survey.possibilities.filter(x => (x.questionId == survey.questions[j].questionId));
            ...
            for (let k = 0; k < possibilitiesLength; k++) {
                if (possibilities[k].possibilityTitle.length < 5) {
                    return "Possibility title must has at least 5 symbols!";
                }
            }
        }
    }
